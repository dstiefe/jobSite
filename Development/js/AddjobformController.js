...
app.directive('contenteditable', function ($sce) {
    ...
    .controller("AddjobformController", function ($scope, Login, $sce, $location, $http) {
        ...
        $scope.id = $location.absUrl().replace($url, "").replace("Jobs/jobmanagement.html?id=", "");
        ...
        if ($scope.id != "Jobs/jobmanagement.html") {
            ...
            $scope.experience = "";
            $scope.employeeType = ""; $scope.jobtype = "";
        ...
        $scope.changeValue = function () {
            if ($scope.locationID > 0) {
                $scope.location = $.grep($scope.locations, function (location) { return location.id == $scope.locationID; })[0].name;
            } else {
                $scope.location = "";
            }
        };
        $scope.saveChanges = function () {
            if (!angular.isUndefined($scope.aboutUsResultContent)) { $scope.aboutUsHtmlContent = $scope.aboutUsResultContent; }
            if (!angular.isUndefined($scope.jobTitleLocationResultContent)) { $scope.jobTitleLocationHtmlContent = $scope.jobTitleLocationResultContent; }
            if (!angular.isUndefined($scope.jobDescriptionResultContent)) { $scope.jobDescriptionHtmlContent = $scope.jobDescriptionResultContent; }
            if (!angular.isUndefined($scope.jobRequirementsResultContent)) { $scope.jobRequirementsHtmlContent = $scope.jobRequirementsResultContent; }
            $scope.jobTitleLocationEditable = false;
            $scope.jobDescriptionContentEditable = false;
            $scope.jobRequirementsContentEditable = false;
            $scope.aboutUsEditable = false;
